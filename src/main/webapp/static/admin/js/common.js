...
/*	//列表添加
   ...
   /* $('#table-list,#table-list2').on('click', '.go-btn', function() {
        var url=$(this).attr('data-url');
        //将iframeObj传递给父级窗口
        parent.page("信息修改", url, iframeObj, w = "500px", h = "600px");
        return false;
    });*/
	//列表删除
	/*$('#table-list,#table-list2').on('click', '.del-btn', function() {
		var sid = $(this).attr('data-id');
		var pageNumber = $(this).attr('lay-data');
		var flag = $(this).attr('data-text');
		var element = $(this).attr('data-title');
		var path = $(this).attr('data-url');
		layer.confirm('确定要删除吗?', {icon: 7, title:'提示'}, function (index) {
                $.post(path,{flag:'deleteById',sid:sid},
                    function(result){
                        ...
                        if(result.error){
                            ...
                            createTable(pageNumber,flag,element);
                        }
                    }
                    ,"text");
            layer.close(index);
